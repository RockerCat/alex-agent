...
**Phase 1A/1B — authenticated headless wake entry point on native Vercel Cron (SolarDesk only, implemented 2026-09-19):** `GET /api/cron/marketing-cycle` (`app/api/cron/marketing-cycle/route.ts`) lets Vercel Cron invoke the existing `runMarketingCycle({ brand: "solardesk", trigger: "scheduled" })` — the same domain function the manual "Run Marketing Cycle" button already calls — without a browser session. Authenticated via Vercel Cron's native mechanism: the endpoint requires `Authorization: Bearer <CRON_SECRET>`, matching the server-side `CRON_SECRET` env var Vercel auto-injects for a scheduled invocation; the endpoint fails closed (refuses to run the agent) if that secret isn't configured, and an unauthenticated request never reaches `agent_runs`, Planner, or any agent state. `vercel.json` configures the SolarDesk wake for once daily at `0 13 * * *` UTC (≈08:00 America/Bogota) — this is agent **wake** cadence only, not a publication cadence; the runtime itself still owns no cron/posting frequency. `CRON_SECRET` must still be configured in the Vercel production environment before the scheduler can successfully invoke the wake — no real scheduled wake has been validated yet. No scheduler beyond this Vercel Cron entry exists; no asset generation or publication automation was added; those remain fully manual, exactly as before. WhatsApp remains unimplemented. The "7 days without opening AlexAgent" milestone has not started.
