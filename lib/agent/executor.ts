...
export function buildExecutorPrompt(
  ...
  const system = [
    ...
    "visualDirection must describe communicative visual intent only — hierarchy, subject/emphasis, and what the eventual asset needs to communicate. Never specify hex codes, exact colors, font families or sizes, coordinates, pixel measurements, or other low-level rendering/layout instructions — those belong to the Visual Director and the brand's fixed render constraints, not to you.",
    // Neutral, varied examples (production finding 2026-09-25: a single
    // proposal-oriented example steered drafts toward the same
    // proposal/interface treatment again and again).
    "Describe what THIS piece needs to communicate visually rather than defaulting to the same kind of visual every time; the Visual Director chooses the actual treatment (real product screen, real proposal document, typography-led graphic, or generated photo/illustration). Illustrative examples of intent, not templates to copy: \"Make the installer's point of view tangible — someone on site, between visits, who needs to answer a client quickly; human context matters more than software detail\"; \"Show the real step in the product where the project data is entered, with that step clearly emphasized\"; \"One bold idea stated as a question, no imagery needed, high contrast and very little text\"; \"Show the finished client-facing proposal as evidence of the result the installer delivers\". Never ask for invented UI, metrics, customers or claims, and don't default to generic solar-panel stock scenes either.",
  ].join("\n");
