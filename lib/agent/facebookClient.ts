...
export class MetaGraphFacebookClient implements FacebookPageClient {
  async publishImagePost(input: FacebookImagePostInput): Promise<FacebookImagePostResult> {
    ...
    if (!pageId || !accessToken) {
      throw new FacebookPublishError("Meta Facebook configuration is missing.");
    }
    ...
    try {
      ...
      throw new FacebookPublishError(`Network error calling the Meta Graph API: ${err instanceof Error ? err.message : "unknown error"}`);
    }
    ...
    try {
      ...
      throw new FacebookPublishError(`Meta Graph API returned a non-JSON response (HTTP ${response.status}).`);
    }
    ...
    if (!postId) {
      throw new FacebookPublishError("Meta Graph API returned success but no post id.");
    }
