...
The intended end-state is that Alex's routine involvement happens over WhatsApp instead of the AlexAgent dashboard: reviewing a proposed publication (including its generated content) and responding approve / reject / request changes; requested changes eventually feeding the existing revision workflow; and any genuine factual/human-input question AlexAgent has being asked (and answered) over WhatsApp, resuming the durable agent run. **Inbound interaction is still future direction, not yet built** — a webhook that accepts Meta's incoming user messages and turns them into approve/reject/question-answer commands, the message protocol, and the persistence architecture for that side are not yet decided. (This is distinct from the outbound delivery-status webhook described below, which only receives Meta's own send-status callbacks and is already implemented.)

**Outbound-only attention notifications are implemented, including production delivery-status diagnostics** (`lib/agent/notifications.ts`, `lib/agent/whatsappClient.ts`, `lib/agent/whatsappWebhook.ts` — see `PROJECT_STATUS.md` for current validated state). Durable invariants that implementation, and any future WhatsApp work (including inbound), must preserve:
...
- The outbound delivery-status webhook is diagnostics-only: it correlates Meta's `sent`/`delivered`/`read`/`failed` callbacks to a `notification_outbox` row by `provider_message_id` and must never mutate `content_drafts`, `agent_questions`, `agent_runs`, or any other workflow table — future inbound-command work is a separate, distinct capability and must not be added into this same route/handler.
